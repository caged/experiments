(function() {
  $(function() {
    return $.getJSON('pub/data/football-recruits-2006-2011.json', function(colleges) {
      var g, h, pb, pl, pr, pt, rules, stars, vis, w, x, y, z, _ref;
      colleges = _.sortBy(colleges, function(d) {
        return d.value.five + d.value.four;
      }).reverse().slice(0, 100);
      stars = d3.layout.stack()(['two', 'three', 'four', 'five'].map(function(stars) {
      ...
      _ref = [20, 20, 100, 50], pt = _ref[0], pr = _ref[1], pb = _ref[2], pl = _ref[3];
      w = 5000;
      h = 600;
      ...
      vis.selectAll('text.label').data(x.domain()).enter().append('svg:text').attr('class', 'label').attr('text-anchor', 'bottom').text(function(d, i) {
        ...
        return "translate(" + (x(d) + (x.rangeBand() / 2)) + ", " + (this.getBBox().width + 10) + ") rotate(-90)";
      });
      ...
      return rules.append('svg:text').attr('x', -25).attr('dy', '0.3em').text(function(d) {
        return d;
      });
    });
  });
